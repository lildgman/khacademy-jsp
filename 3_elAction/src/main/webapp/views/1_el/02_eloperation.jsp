...
<body>
	...
	sOne과 sTwo가 일치하나요? ${sOne == sTwo }; 또는 ${sOne eq sTwo } <br>
	sOne과 sTwo가 일치하지 않습니까? ${sOne != sTwo }; 또는 ${sOne ne sTwo } <br>
	
	big에 담긴 값이 10과 일치합니까? ${big == 10 } 또는 ${big eq 10 } <br>
	
	sThree에 담긴 값이 '안녕' 과 일치합니까? ${sThree == '안녕' } <br><br>
	<!-- el구문 안에서 문자열 리터럴값은 홀따옴표 쌍따옴표를 가리지 않는다 -->
	
	<h4>4. 객체가 null인지 아닌지 리스트가 비어있는지 비교</h4>
	pTwo가 null 인가? ${pTwo == null } 또는 ${pTwo eq null } 또는 ${empty pTwo } <br>
	pOne가 null 인가? ${pOne == null } 또는 ${pOne eq null } 또는 ${empty pOne } <br>
	pOne가 null 인가? ${pOne != null } 또는 ${pOne ne null } 또는 ${not empty pOne } <br>
	
	aOne이 텅 비어있나요? ${empty aOne }; <br>
	aTwo이 텅 비어있나요? ${empty aTwo };
